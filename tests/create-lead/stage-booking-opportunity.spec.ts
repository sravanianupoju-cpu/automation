...
/**
 * Verified flow (Stage = Booking Opportunity):
 ...
 */
test.describe('Create Lead - Stage: Booking Opportunity', () => {
  test('creates a lead with Stage = Booking Opportunity using only mandatory fields', async ({
    createLeadPage,
  }) => {
    const mobileNumber = randomIndianMobileNumber();
    const leadName = uniqueLeadName('AutoBookOpp');

    await test.step('Navigate to Create Lead page', async () => {
      await createLeadPage.open();
    });

    await test.step('Fill mandatory Contact details and select Stage = Booking Opportunity', async () => {
      await createLeadPage.fillContactDetails({
        name: leadName,
        mobileNumber,
        project: PROJECT_NAME,
        stage: LeadStage.BookingOpportunity,
      });
